...
Utils.getWeather = function(pos) {
    try {
        var data0 = org.jsoup.Jsoup.connect("https://m.search.daum.net/search?q=" + pos.replace(/ /g, "+") + "+날씨").get();
        var data = data0.select("div.info_main").get(0);
        var status = data.select("p").text() + "";
        var temp = data.select("em.txt_temp").text() + "";
        var data = data0.select("ul.list_detail").select("li");
        var hum, wind, dust;
        for (var n = 0; n < 8; n++) {
            var value = data.get(n).select("span.txt_num").text().trim();
            if (value == "") value = "(정보 없음)";
            switch (data.get(n).select("span.txt_type").text().trim()) {
                case "습도":
                    hum = value;
                    break;
                case "풍속":
                    wind = data.get(n).select("span.txt_state").text().trim().split(" ")[0] + ", " + value;
                    break;
                case "미세먼지":
                    dust = value;
                    break;
        ...
        var result = "[" + pos + " 날씨 정보]\n" +
            "상태 : " + status.split(", 어제")[0] + "\n" +
            "온도 : " + temp + "\n" +
            "습도 : " + hum + "\n" +
            "바람 : " + wind + "\n" +
            "미세먼지 : " + dust;
        result = result.replace("°c", "℃").replace(" ㎍/㎥", "μg/m³");
        return result;
    } catch (e) {
        return null;
    }
...
Utils.getBitcoinPrice = function() {
    return org.jsoup.Jsoup.connect("https://m.search.daum.net/kakao?w=tot&DA=SH1&q=%EB%B9%84%ED%8A%B8%EC%BD%94%EC%9D%B8%20%EC%8B%9C%EC%84%B8%201%EC%9D%BC&rtmaxcoll=EMA").get().select("span.txt_price").get(0).text();
};
