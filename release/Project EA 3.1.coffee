...
<설명>
 - EA는 '에아'라고 읽습니다.
 - 채팅 자동응답 봇에서 작동한다고 가정하고 만들어진 봇입니다.
<가이드라인>
...
<라이선스>
 - 이 소스에는 GPL 3.0이 적용되어있습니다.
    ...
    Project EA Copyright (C) 2018-2021 Dark Tornado
...
EA.say = (msg, replier) -> replier.reply "[EA] " + msg;
EA.debug = (str) -> 
    try
        Log.debug str
    catch
...
Utils.getCurrentWeather = () -> 
    try
        data = org.jsoup.Jsoup.connect("https://m.search.naver.com/search.naver?query=날씨").get()
        data = data.select("div.lcl_lst").get(0).select "a"
        result = [];
        for n in [0..data.size()-1]
            dd = data.get(n).select "span"
...
Utils.getDustData = () -> 
    try
        data = Utils.parse "https://m.search.naver.com/search.naver?query=미세먼지"
        ...
        for dd, n in data
            if n%2 is 0
                ...
                result += Utils.dustLevel(Number(data[n])) + " (" + data[n] + "μg/m³)\n"
        result.trim()
    catch e
...
Utils.dustLevel = (value) -> 
    ...
    "매우나쁨"
Utils.getWeather = (pos) -> 
    try
        data0 = Utils.parse "https://m.search.naver.com/search.naver?query=" + pos.replace(/ /g, "%20") + "%20날씨"
        ...
        print e
        null
Utils.getBitcoinPrice = () -> Utils.parse("https://m.search.daum.net/kakao?w=tot&DA=SH1&q=%EB%B9%84%ED%8A%B8%EC%BD%94%EC%9D%B8%20%EC%8B%9C%EC%84%B8%201%EC%9D%BC&rtmaxcoll=EMA").select("span.current_stock").get(0).text()
Utils.getMapleInfo = (name) -> 
    try
        ...
        guild = data.select("a").text()
        rank = data.get(1).select("span").get(0).text()
        "[메이플스토리 캐릭터 정보]\n" +
        "닉네임 : " + name + "\n" +
        "레벨 : " + lv.replace("Lv.", "") + "\n" +
        "직업 : " + job + "\n" +
...
procCmd = (room, msg, sender, isGroupChat, r) -> 
    ...
    if cmd is "따라하기"
        EA.say data, r
    if cmd is "파싱"
        result = Utils.getWebText data
        EA.say "파싱 결과입니다.\n" + result, r
    if msg is "날씨"
        result = Utils.getCurrentWeather()
        EA.say "현재 날씨입니다.\n" + result, r
    else if cmd is "날씨"
        result = Utils.getWeather data
        if result is null
            EA.say "날씨 정보 불러오기 실패", r
        else
            EA.say "[" + data + " 날씨 정보]\n" + result, r
    if msg is "호출"
    ...
    if cmd is "번역"
        ...
        result = Api.papagoTranslate lang1, lang2, value
        EA.say "번역 결과입니다.\n" + result, r
    if msg is "시간"
        day = new Date()
    ...
    if msg is "날짜"
        day = new Date()
        EA.say "오늘은 " + (day.getMonth() + 1) + "월 " + day.getDate() + "일입니다.", r
    if msg is "주사위"
        icon = ["⚀", "⚁", "⚂", "⚃", "⚄", "⚅"]
